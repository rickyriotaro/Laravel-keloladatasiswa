<x-app-layout>
    ...
    <div class="container mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold mb-6">Edit Wali Kelas</h1>

        <form action="{{ route('walikelas.update', $walikelas->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="guru_id" id="guru_id" required
                    ...
                    @foreach ($guru as $g)
                        <option value="{{ $g->id }}" {{ $walikelas->guru_id == $g->id ? 'selected' : '' }}>
                            {{ $g->nama_guru }}
                        </option>
                    @endforeach
                </select>
            </div>
<!-- Tambahkan dropdown user_id -->
<div class="mb-4">
